...
// Проверяем, что элементы существуют перед работой с ними
if (datePicker && dateButton) {
    ...
    if (datePicker.value) {
        ...
        if (parts.length === 3) {
            ...
            if (dateDisplay) updateDisplayDate(currentDate);
        }
    }
    ...
    if (!currentDate) {
        ...
        if (dateDisplay) updateDisplayDate(today);
    }
    ...
    const fp = flatpickr(datePicker, {
        ...
        onChange: function(selectedDates, dateStr) {
            if (selectedDates.length > 0) {
                ...
                if (dateDisplay) updateDisplayDate(selectedDate);
